...
`17.5MB * 86400 * 365 = 551T`

### Capasity - Пост (метаинформация)

`1KB * 86400 * 365 = 30GB`

### Capasity - Комментарий (медиа)

`345KB * 86400 * 365 = 10.9TB`

### Capasity - Комментарий (метаинформация)

`9KB * 86400 * 365 = 170GB`

### Capasity - Реакция (метаинформация)

`750KB * 86400 * 365 = 22TB`

### Capasity - Общее (медиа)

`552TB + 10.9TB = 563TB`

### Capasity - Общее (метаинформация)

`30GB + 170TB + 22TB = 22.2TB`

### iops HDD - Пост (медиа) 

`12000 RPS * 3(среднее кол-во фото в посте, а значит 3 доп. похода) / 100 iops = 360 Дисков`

### iops HDD - Пост (метаинформация)

`12000 RPS / 100 iops = 120 Дисков`

### throughput HDD - Пост (медиа)

`17.5MB/s / 100MB/s = 1 Диск`

### throughput HDD - Пост (метаинформация)

`1KB/s / 100MB/s = 1 Диск`

### iops HDD - Комментарий

`6000 RPS / 100 iops = 60 Дисков`

### throughput HDD - Комментарий (медиа)

`345KB/s / 100MB/s = 1 Диск`

### throughput HDD - Комментарий (метаинформация)

`9KB/s / 100MB/s = 1 Диск`

### iops HDD - Реакция

`600 RPS / 100 iops = 6 Дисков`

### throughput HDD - Реакция (метаинформация)

`750KB/s / 100MB/s = 1 Диск`

### iops SSD - Пост (медиа)

`12000 RPS * 3(среднее кол-во фото в посте, а значит 3 доп. похода) / 1000 iops = 36 Дисков`

### iops SSD - Пост (метаинформация)

`12000 RPS / 1000 iops = 12 Дисков`

### throughput SSD - Пост (медиа)

`17.5MB/s / 500MB/s = 1 Диск`

### throughput SSD - Пост (метаинформация)

`1KB/s / 500MB/s = 1 Диск`

### iops SSD - Комментарий

`6000 RPS / 1000 iops = 6 Дисков`

### throughput SSD - Комментарий (медиа)

`345KB/s / 500MB/s = 1 Диск`

### throughput SSD - Комментарий (метаинформация)

`9KB/s / 500MB/s = 1 Диск`

### iops SSD - Реакция

`600 RPS / 1000 iops = 1 Дисков`

### throughput SSD - Реакция (метаинформация)

`750KB/s / 500MB/s = 1 Диск`

### Конфигурация

Capasity в дисках для медиа `563TB / 0.85(15% объема диска как правило недоступны) = 670TB`

Capasity в дисках для метаинформации `22.2TB / 0.85(15% объема диска как правило недоступны) = 26TB`

Для медиафайлов будем использовать amazon s3.

Для метаинформации выберем `SSD sata диски`, нужно `17 Дисков по 2TB`
Для медиа выберем `SSD sata диски`, нужно `36 дисков по 20 ТБ`, это даже чуть с запасом

## Репликация

Для медиаинформации выберем ceph, replication factor = 2

Будем использовать 1 диска на хост, `36 * 2 = 72 хоста`

- `/avatars/{user_id[:2]}/{user_id}/avatar_{size}.jpg`
- `/posts/{user_id[:2]}/{user_id}/{post_id}/{photo_id}.jpg`

- {user_id[:2]} – `первые два символа user_id, чтобы разгрузить "горячие" OSD (равномерное распределение).`
- {user_id} – `пользователь, чтобы легко находить все его фото.`
- {post_id} – `ID поста (если фото привязано к посту).`
- {photo_id} – `UUID - ID конкретного фото.`
- {size} – `если нужны превьюшки (small, medium, original).` // можно реализовать не в первой итерации.

Для метаинформации выберем postgres, async replication, master/slave, replication factor = 3

Будем использовать 2 диска на хост, `17 / 2 * 3 = 26 хостов`

- Комментарий - `key based way by post_id`, применяем к `post_id` хэш функцию.
- Пост - `key based way by user_id`, применяем к `user_id` хэш функцию.
- Реакция - `key based way by post_id`, применяем к `post_id` хэш функцию.
- Места отдыха - на отдельном хосте, предподготовленная таблица.
